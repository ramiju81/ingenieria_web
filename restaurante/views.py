...
def crear_pedidos(request):
    if request.method == 'POST':
        ...
        form = ProductoForm()
    return render(request, 'restaurante/crear_pedido.html', {'form': form})

def listar_pedidos(request):
    productos = Producto.objects.all().order_by('-created_at')
    return render(request, 'restaurante/lista_pedidos.html', {'productos': productos})

def ver_factura(request):
    productos = Producto.objects.all().order_by('-created_at')
    return render(request, 'restaurante/ver_factura.html', {'productos': productos})
